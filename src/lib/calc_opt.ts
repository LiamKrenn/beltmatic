...
export function leastSteps(target: number, max_src: number, allowedOperators: string[]) {
	...
	function skipForward(value: number, operator: string): boolean {
		if ((operator === '+' || operator === '*' || operator === '^') && value > target) {
			return true;
		}
		if ((operator === '-' || operator === '/') && value < target) {
			return true;
		}
		return false;
	}

	function skipBackward(value: number, operator: string): boolean {
		if ((operator === '-' || operator === '/' || operator === '^') && value < 0) {
			return true;
		}
		return false;
	}
	...
	while (forwardQueue.length > 0 && backwardQueue.length > 0) {
		...
		for (let i = 0; i < backwardSize; i++) {
			...
			for (const num of allowedNumbers) {
				for (const operator of allowedOperators) {
					if (skipBackward(element.value, operator)) continue;
